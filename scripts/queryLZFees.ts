...
import {
  mainnetChainIdToLZInfo,
  testnetChainIdToLZInfo,
} from "../src/crosschain/lz-networks";
import { networkIdToName } from "../src/networks";

import type { ILayerZeroEndpoint } from "../typechain-types";

// FIXME: does not work, looks like `hardhat-change-network` is not compatible with the latest `hardhat`
async function main(): Promise<void> {
  ...
  const chainIdToLZInfo = isMainnet
    ? mainnetChainIdToLZInfo
    : testnetChainIdToLZInfo;
  ...
  for (const [chainId, lzInfo] of chainIdToLZInfo.entries()) {
    ...
    for (const [__, lzInfo] of chainIdToLZInfo.entries()) {
      ...
      try {
        ...
        const feeRoundedStr =
          notRoundedPart + "0".repeat(feeStr.length - notRoundedPart.length);
