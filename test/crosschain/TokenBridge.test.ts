...
describe("TokenBridge", function () {
  ...
  let RootToken: TestERC20;
  let ChildToken: TestERC20;

  beforeEach(async () => {
    [Admin, Bridger, OtherBridger, RootTokenBridger, Someone] =
    ...
    RootToken = (await TestERC20Factory.deploy(
      "RootToken",
      "RTN",
      8,
      1_000_000
    )) as TestERC20;
    ChildToken = (await TestERC20Factory.deploy(
      "ChildToken",
      "CTN",
      8,
      1_000_000
    ...
    // mint Root tokens
    await RootToken.mint(Bridger.address, startBalance);
    ...
    await RootBridge.grantRole(
      utils.keccak256(encodedRootTokenAddress),
      RootTokenBridger.address
    );
    await ChildBridge.grantRole(
      utils.id(ChildToken.address),
      RootTokenBridger.address
    );
  });
  ...
  describe("view", () => {
    describe("getDstToken", () => {
      it("return correct dst token", async () => {
        ...
        expect(
          await RootBridge.getDstToken(RootToken.address, chainId)
        ).to.equal(ChildToken.address);
      });
  ...
  describe("modification", () => {
    ...
    describe("removeToken", () => {
      ...
      it("revert on not admin removing token", async () => {
        await expect(
          RootBridge.connect(Someone).removeToken(RootToken.address)
        ).to.be.revertedWith(ACCOUNT_MISSING_ROLE(Someone.address, ADMIN_ROLE));
      });
    ...
    describe("setDstToken", () => {
      ...
      it("admin can add non-zero dst token", async () => {
      ...
      it("admin can add zero dst token", async () => {
        await RootBridge.connect(Admin).setDstToken(
          RootToken.address,
          chainId,
          ChildToken.address
      ...
      it("revert on not admin setting dst token", async () => {
        await expect(
          RootBridge.connect(Someone).setDstToken(
            RootToken.address,
            chainId,
            ChildToken.address
          )
      ...
      it("revert on adding dst token to not supported token", async () => {
        await expect(
          RootBridge.setDstToken(ChildToken.address, chainId, RootToken.address)
        )
          .to.be.revertedWithCustomError(RootBridge, "TokenNotSupported")
          .withArgs(ChildToken.address);
      });
      ...
      it("event is emitted", async () => {
        await expect(
          RootBridge.setDstToken(RootToken.address, chainId, ChildToken.address)
        )
  ...
  describe("bridge", () => {
    beforeEach(async () => {
      ...
      await ChildBridge.setDstToken(
        ChildToken.address,
        chainId,
        RootToken.address
      );

      // NOTE: in TestERC20 minting does not require any role
      // grant MinterRole to ChildBridge on ChildToken
      // await ChildToken.grantRole(
      //   await ChildToken.MINTER_ROLE(),
      //   ChildBridge.address
      // );
    });
    ...
    describe("success", () => {
      it("bridger role can bridge", async () => {
        // initial balances
        expect(await RootToken.balanceOf(Bridger.address)).to.be.equal(
          startBalance
        );
        ...
        expect(await RootToken.balanceOf(RootBridge.address)).to.be.equal(
          amount
        );
      ...
      it("bridging lock tokens on RootBridge, does not mint to ChildBridge", async () => {
        // approve tokens for RootBridge
        await RootToken.connect(Bridger).approve(RootBridge.address, amount);
      ...
      it("token bridger can bridge their token", async () => {
        ...
        // initial balances
        expect(
          await RootToken.connect(RootTokenBridger).balanceOf(
            RootTokenBridger.address
          )
        ).to.be.equal(startBalance);

        // approve tokens for RootBridge
        await RootToken.connect(RootTokenBridger).approve(
          RootBridge.address,
          amount
        );
      ...
      it("bridge out Child: tokens are burned on Child and unlocked on Root", async () => {
        ...
        expect(await ChildToken.balanceOf(Bridger.address)).to.be.equal(
          startBalance - amount
        );
      });
      ...
      it("can bridge to other account", async () => {
        ...
        expect(await ChildToken.balanceOf(OtherBridger.address)).to.be.equal(
          amount
        );
      });
    ...
    describe("revert", () => {
      ...
      it("revert when bridging by token bridger of other token", async () => {
        ...
        // NOTE: in TestERC20 minting and burning does not require any role
        // await RootToken.grantRole(
        //   utils.keccak256(encodedChildTokenAddress),
        //   Someone.address
        // );
      ...
      it("revert when bridging to not bridger role", async () => {
        ...
        const path = (
          RootBridge.address + ChildBridge.address.slice(2)
        ).toLowerCase();
